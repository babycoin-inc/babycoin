...
const insertTransaction = (client, transaction, orderType) => {
  ...
  return client.query('insert into transactions (order_type, currency, purchase_price, total_trade_fiat, total_trade_coin, order_datetime, trader_id, coin_id) values ($1, $2, $3, $4, $5, now() , $6, $7)', recordToCreate)
...
exports.fulfillBuyAllTransaction = async (transaction, user_id) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    // Subtract total_trade_fiat and update dollar cost for CASH
    client.query(`UPDATE portfolio SET quantity = 0, dollar_cost = 0 WHERE coin_id = (SELECT id FROM coins WHERE acronym = 'usd') AND trader_id = $1;`, [user_id]);
    // add total_trade_coin to portfolio || Dollar Cost || Avg Price
    client.query(`INSERT INTO portfolio(trader_id, coin_id, dollar_cost, avg_price, quantity) VALUES($1, $2, $3, $4, $5) ON CONFLICT (trader_id, coin_id) DO UPDATE SET quantity = (portfolio.quantity + $5), dollar_cost = (portfolio.dollar_cost + $3), avg_price = ((portfolio.dollar_cost + $3) / (portfolio.quantity + $5)) WHERE portfolio.coin_id = $2 AND portfolio.trader_id = $1;`, [user_id, transaction.coin_id, transaction.total_trade_fiat, transaction.purchase_price, transaction.total_trade_coin]);
    await client.query('COMMIT');
    return 'Buy Transaction Completed';
    ...
    await client.query('ROLLBACK');
    console.log('error inserting buy record')
    console.error(err);
  } finally {
    client.release();
  }
...
exports.fulfillSellAllTransaction = async (transaction, user_id) => {
  ...
  try {
    await client.query('BEGIN');
    // ADD INITIAL PORTION OF TRANSACTION HERE
    const sellOrder = await insertTransaction(client, transaction, 'sell');
    // add total_trade_fiat from cash
    client.query(`UPDATE portfolio SET quantity = (quantity + $1), dollar_cost = (dollar_cost + $1) WHERE coin_id = (SELECT id FROM coins WHERE acronym = 'usd') AND trader_id = $2;`, [transaction.total_trade_fiat, user_id]);
    // subtract coin.id from portfolio || Dollar Cost || Avg Price
    client.query(`DELETE FROM portfolio WHERE coin_id = $1 AND trader_id = $2;`, [transaction.coin_id, user_id]);
    await client.query('COMMIT');
...
exports.fulfillSellTransaction = async (transaction, user_id) => {
  ...
  try {
    ...
    client.query(`UPDATE portfolio SET quantity = (quantity - $1), dollar_cost = (dollar_cost - ($1 * $2)), avg_price = (dollar_cost - ($1 * $2)) / (quantity - $1) WHERE coin_id = $3 AND trader_id = $4;`, [transaction.total_trade_coin, transaction.purchase_price, transaction.coin_id, user_id]);
    await client.query('COMMIT');
    return 'Hit Fulfill Sell Transaction';
  } catch (err) {
    await client.query('ROLLBACK');
